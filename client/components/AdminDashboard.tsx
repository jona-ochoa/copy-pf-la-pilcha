...
const AdminDashboard: React.FC = () => {
  ...
  const handleToggleBanUser = async (userId: string, isBanned: boolean) => {
    try {
      const response = await axios.put(`http://localhost:3001/api/v1/user/${userId}`, { isBanned });
  ...
  const handleToggleDeactivateProduct = async (productId: string, isDeactivated: boolean) => {
    try {
      const response = await axios.put(`http://localhost:3001/api/v1/products/${productId}`, { isDeactivated });
      console.log(response.data.message);
  ...
  const handleToggleAdmin = async (userId: string, isAdmin: boolean) => {
    try {
      const response = await axios.put(`http://localhost:3001/api/v1/user/${userId}`, { isAdmin });
      console.log(response.data.message);
